...
import { removeItem, toggleAmount } from "../slices/cartSlice";
import { useAppDispatch } from "../hooks";
import { CartItemType } from "../utils/types";

const CartItem = (item: CartItemType) => {
  const { cartID: id, amount, title, price, image, productColor: color } = item;

  const dispatch = useAppDispatch();

  const increase = () => {
    dispatch(toggleAmount({ id: parseInt(id), value: "inc" }));
  };

  const decrease = () => {
    dispatch(toggleAmount({ id: parseInt(id), value: "dec" }));
  };
